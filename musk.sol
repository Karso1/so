...
contract MuskNFT is Context, IERC20, IERC20Metadata, Ownable{
    ...
    address public marketAddress = 0xb98c8bF051014aFC51e806F568c45BB70CB387D0;
    address public usdtMintAddress = 0x55d398326f99059fF775485246999027B3197955;
    
    constructor () {
        _mint(_msgSender(), 10000000 * 1e18);
        emit Transfer(address(0), _msgSender(), 10000000 * 1e18);
    ...
    function transferFrom(
        ...
        require(currentAllowance >= amount, "BET: transfer amount exceeds allowance");
        unchecked {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "BET: decreased allowance below zero");
        unchecked {
    ...
    function _transfer(
        ...
        require(sender != address(0), "BET: transfer from the zero address");
        require(recipient != address(0), "BET: transfer to the zero address");
        require(!isBot[sender],"BET: robots can't trade");
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "BET: transfer amount exceeds balance");
        uint256 _amount = amount;
        if (sender!=marketAddress&&recipient!=marketAddress) {
            _balances[marketAddress] = _balances[marketAddress].add(amount.div(1000).mul(30));
            emit Transfer(sender, marketAddress, amount.div(1000).mul(30));
            _amount = amount.div(1000).mul(970);
        }
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "BET: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "BET: burn amount exceeds balance");
        unchecked {
   ...
   function setBot(address addr,bool t)public onlyOwner{
        isBot[addr]=t;
    }
